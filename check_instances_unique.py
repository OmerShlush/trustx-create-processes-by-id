...
def main():
    ...
    for instance in completed_success_instances:
        ...
        created_dtm = instance_details.get("createdDtm")
        ...
        if user_data:
            ...
            record = {
                ...
                "created DTM": created_dtm,
                "Document Number": document_number,
                "Valid ID": valid_id
            }

            if str(valid_id) == str(document_number) and document_number is not None:
                matching_ids.append(record)
            else:
                non_matching_ids.append(record)

    matching_df = pd.DataFrame(matching_ids)
    non_matching_df = pd.DataFrame(non_matching_ids)

    if not matching_df.empty:
        matching_df = matching_df.sort_values(by="created DTM")
    if not non_matching_df.empty:
        non_matching_df = non_matching_df.sort_values(by="created DTM")

    if not matching_df.empty:
        summary_df = matching_df.groupby(["Document Number", "Valid ID"]).size().reset_index(name="Matching Count")
    else:
        summary_df = pd.DataFrame(columns=["Document Number", "Valid ID", "Matching Count"])

    today_str = datetime.today().strftime("%d-%m-%Y")
    filename = f"process_instances_insights_with_summary_{today_str}.xlsx"
    with pd.ExcelWriter(filename) as writer:
        matching_df.to_excel(writer, sheet_name="Matching IDs", index=False)
        non_matching_df.to_excel(writer, sheet_name="Non-Matching IDs", index=False)
        summary_df.to_excel(writer, sheet_name="Matching Summary", index=False)

    print(f'Enhanced spreadsheet created: {filename}')
